...
export const DOCS_ACCESS_COOKIE_NAME = 'ddmusic_docs_access';
export const DOCS_ACCESS_MAX_AGE_SECONDS = 60 * 60 * 12;
...
export const createDocsAccessToken = (expiresAt: number): string => {
  const payloadBase64 = Buffer.from(JSON.stringify({
    scope: 'docs',
    issuedAt: Date.now(),
    expiresAt,
  } satisfies DocsAccessPayload), 'utf8').toString('base64url');
  const signature = sign(payloadBase64);
  return `${payloadBase64}.${signature}`;
};

export const verifyDocsAccessToken = (token?: string | null): boolean => {
  if (!token) {
    return false;
  }
  ...
  try {
    const payload = JSON.parse(Buffer.from(payloadBase64, 'base64url').toString('utf8')) as DocsAccessPayload;
    return payload.scope === 'docs' && payload.expiresAt > Date.now();
  } catch {
    return false;
  }
};
